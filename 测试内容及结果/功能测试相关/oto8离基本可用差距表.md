...
#### 新增问题
- 权限管理camera部分
   - 微信的拍照、视频等功能无摄像头数据（可能用的是ndk）
   - 微博的扫一扫功能中，摄像头无法自动生效权限修改
   - 微博、钉钉等显示的摄像头图像方向不对，有时是倒置的，有时是横着的
   - qq、钉钉都存在使用时闪退的问题
   - s1、s6笔记本上摄像头不可用（s6在ubuntu上可拍照、s1在ubuntu和windows上也不能拍照）
   - 部分其它应用的权限依赖于相机的权限。即其它应用开了摄像头权限，而相机没开摄像头权限，其它应用也不能正常使用摄像头。（比如微信的视频通话功能未接听时）
- 多窗口：印象笔记的拍照、一体机的照相机-摄像、钉钉的扫一扫中，摄像头显示画面亮度过高且半透明（能看到桌面图标），但是拍出的画面是正常的
***
...
- camera部分
   ...
   - 微博、钉钉等显示的摄像头图像方向不对，有时是倒置的，有时是横着的
   - qq、钉钉都存在使用时闪退的问题
   - s1、s6笔记本上摄像头不可用（s6在ubuntu上可拍照、s1在ubuntu上也不能拍照）
   - 部分其它应用的权限依赖于相机的权限。即其它应用开了摄像头权限，而相机没开摄像头权限，其它应用也不能正常使用摄像头。（比如微信的视频通话功能未接听时）
...
- 微信问题：
   ...
   - 微信（704）全屏回到竖屏模式后，手机模式上方小程序区域显示异常，无法恢复
   - 微信（705）跳一跳小程序无法使用
   - 微信（705）全屏回到竖屏模式后，朋友圈播完视频会卡住
