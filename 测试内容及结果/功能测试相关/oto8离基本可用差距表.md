...
- 需要屏蔽的功能：热点和网络共享、VPN、飞行模式、应用权限、默认应用、特殊应用权限、显示>自动旋转屏幕、屏保、有来电时响铃并震动、“勿扰“偏好设置、手机铃声、拨号键盘提示音、充电提示音、安卓x86设置、存储、Google Play保护机制、安全更新、屏幕固定、Google
...
- 任务管理器 
- 权限管理器
   - camera部分：加载虚拟摄像头遇到困难，计划用新方案：在内核层面添加虚拟摄像头-----------相机的框架已看明白，正在着手修改
- 文本编辑器
- 视频播放器
- 打印
   - mopria打印功能移植
...
## 按工程师分类
#### 肖络元
- 应用商店后台调研-----------与WZX沟通，计划使用PHP+MySQL的方式构建，最后配合他上线到服务器即可。
- 无法U盘启动系统的bug调试------------已找到问题，预计今天提交
- 休眠问题，蓝牙系统底层
   - 分析蓝牙扫描不到设备的原因-----------定位到问题出在HAL层,正在向黄Sir寻求帮助
- 基于musl libc，移植Linux GNU常用命令工具到openthos8.1
