...
- 选择一个文件夹 -> 下一级 -> 前进，提示xx 文件夹已经删除
- 选择一个文件夹 -> 下一级 -> 前进 -> 下一级菜单，提示xx文件夹已经删除，并且不能进入下一级文件夹
## 2019-08-23
- 文档的名字过长会遮挡进度条
- 进度条显示的文件大小不对，例如显示1.2G，但是在文件管理器显示1.13G.过小的文件显示0K
- 部分文件在进度条显示还没下载完，但是在文件管理器显示已经下载完。例如停在414.1MB/414.8MB
- 同时下载两个文件，在列表里只显示了一个
## 2019-08-27
- 空文件夹不能下载
...
- 点击进度条右边的打开文件崩溃
## 2019-09-06
- 第一次登录其他openthos服务器，网盘地址显示不对，http://android.accounts.AccountManager@5ad4f6e
- 点击存放路径下的浏览按钮崩溃。
- 下载失败出现了一个暂停按钮
