...
9. UI界面未重新实现(开始菜单、任务栏、通知中心、文件管理器等)

10. 云服务：文件管理器、设置和首次配置的云服务未移植
...
13. 设置："用户和账号/用户"里切换为访客，则桌面和控制栏无法访问。

14. 设置：以太网功能未移植

15. 设置：显示里“显示缩放”功能未移植(即调整分辨率)
...
20. 文件管理器：永久删除和回收站的清空功能不可用

21. 文件管理器：在自动挂载里点击其它分区会使文件管理器崩溃

22. 文件管理器：工具栏设置按钮的云服务信息不可用

23. 文件管理器：点击右键菜单的“发送”选项使文件管理器崩溃
...
27. SystemUI：开始菜单搜索任意内容，随后安装一个应用，再打开开始菜单可以看到那个应用。此时删除搜索框里的字符，新安装应用不显示在开始菜单里。

28. 浏览器：打开outlook.com后崩溃

29. 浏览器：打开github.com个人页面，点击Sarts、Followers等链接进入"Aw,Snap!"页面

30. 浏览器：无法播放视频和音频，html5也无法播放

31. 浏览器：页内搜索的“上一条”和“下一条”按钮无效。
...
1. 快捷键未移植
2. 硬件问题：S6笔记本上摄像头不可用（s6在ubuntu上可拍照）
3. 内置Linux：基于musl libc，移植Linux GNU常用命令工具到openthos8.1
4. 多窗口：窗口播放两个视频，两个视频分别设置不同的亮度。点击哪个视频，系统就会跟随哪个视频的亮度。
5. 多窗口：当其它app中视频正在播放的时候，在设置里调节屏幕亮度的功能失效。而按住左键在视频的左边滑动可控制桌面的亮度。bilibili、学堂在线、微博均可复现此问题。
6. 设置：蓝牙匹配新设备时，有时需要点击”与新设备匹配”按钮多次才能进入扫描页面
7. 设置：界面锁定的时候解锁的有效区域是通知中心所在的区域
8. 设置：界面锁定的时候解锁的方式为上滑解锁，这只适用于触屏设备，与通常的桌面操作不符。
9. 文件管理器：搜索功能不可用
10. 文件管理器：工具栏设置按钮的"显示隐藏文件"选项无效，但右键菜单里的“显示隐藏文件”选项是有效的
11. 文件管理器：网上邻居里看不到其它Openthos(8和2)共享出来的文件
12. 文件管理器：工具栏中的“开启共享”功能需要点击两次才有效
13. 文件管理器：tab键选中一个按钮，左键选中一个文件，此时按回车键会产生点击按钮的操作。这种“选中”操作应该是互斥的。
14. 文件管理器：右键菜单排序后，一级菜单不消失
15. 文件管理器：排序：时间排序不生效
16. 文件管理器、桌面，拷贝较大的视频时，alert框显示的是上一次拷贝的内容
17. 桌面：使用右键(点击空白或新建文件等)，整个屏幕会闪一下
18. 桌面：图标右键选打开后界面会先黑一下再打开新窗口
19. 应用商店：安装应用时卸载应用，系统会直接软重启
20. SystemUI：通知中心CLEARALL按钮对有些消息有效果，有些没有效果。以前版本的处理方式是：弹出可以清除的消息时显示clearall按钮，对不可清除的消息不显示clearall按钮。
21. 浏览器：不能复制网址
22. 浏览器：设置浏览器的默认页为openthos.com
23. 浏览器：百度搜索框的下拉菜单无法选择搜索项，自带的chrome浏览器无此问题
24. 浏览器：网页版微信无操作20分钟后断开连接，ubuntu上无此问题，自带的chrome浏览器也无此问题
25. 浏览器：腾讯文档无法创建和打开文档
26. 浏览器：在S1本上，用触摸板点击“功能”键(三个竖点)，会同时点击到弹出菜单里对应位置的选项(移动到其它窗口)，鼠标点击无此问题，T45本上无此问题
27. 浏览器应默认打开桌面版网页
